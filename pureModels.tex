...
\NI We use node-labelled transition systems as models for
\cathoristic{}. The purpose of the node labels is to express
...
to giving semantics to $!A$ which does not require node-labelling: we
simply check if all actions of all outgoing transitions at the current
state are in $A$.  As the sematnics of other formula requires node
labelling in its satisfaction condition, this means we can use plain
...
\subsection{Pure cathoretic models}
...
\subsection{A semantic characterisation of elementary equivalence for 
