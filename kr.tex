...
The entire world state is stored as a set of sentences - all other aspects of world state are transient and computed as needed.
...
Consider the following facts about Brown\footnote{The database is composed of Y-terms. For readability, the trailing $\star \top$ has been omitted. In Praxis, constants are identifiers starting with a lower-case letter while variables start with an upper-case letter.}:
\begin{verbatim}
brown *
    (
        sex!male /\
...
(Compare this with Prolog - where it is much harder to remove all sentences containing a particular symbol).
A sub-tree of terms is the Eremic Logic equivalent of an \emph{object} in an object-oriented programming language.

The tree-structure of terms also allows us to express the \emph{life-time of data} in a natural way. 
...
\end{verbatim}
In Eremic Logic, if we want to find out whether Brown is married, we can query the sub-term directly -  we just ask if 
\begin{verbatim}
brown * married
\end{verbatim}
...
Since someone can only have a single spouse at any moment, the rule is expressed in Eremic Logic as\footnote{Note that the rule is a horn-clause of Y-terms, not X-terms. Note also that the trailing $\star \top$ has been suppressed for readability.}:
\begin{verbatim}
married_couple_at_home * X * Y :-
    at_home * X,
    at_home* Y,
