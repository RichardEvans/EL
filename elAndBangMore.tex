...
unique to \ELABR{}.  Standard rules are [\RULENAME{Id}],
...
Relatedly, the rule [\RULENAME{Normal}] enables us to
...
That the logic has no axioms. One reason for a purely rule-based
...
\martin{Where? Not in this section? Isn't $\BIGLUB$ explained as 
part of the general bla on quotienting?}
...
\NI The functions $\mathsf{bang}(\cdot)$ and $\mathsf{trans}$\martin{We don't seem to be using $\mathsf{trans}$} on
...
\martin{There is some confusion about lemma numbers. Which lemma are we proving here?}
