...
\emph{characteristic formula} of a model.  
The function $\SIMPL{\cdot}$ takes formulae
as argument and returns models. Characteristic formulae go the other
way: given a model $\MMM$, we compute its characteristic formula
$\CHAR{\MMM}$. The formula $\CHAR{\MMM}$ must satisfy the following laws to characterise the model
$\MMM$:
\begin{eqnarray*}
\MMM \models \CHAR{\MMM} \\
\forall \phi . \text{ if } \MMM \models \phi \text{ then } \CHAR{\MMM} \models \phi \\
\forall \psi . \text{ if } (\forall \phi . \text{ if } \MMM \models \phi \text{ then } \CHAR{\MMM} \models \phi) \text{ then } \psi \models \CHAR{\MMM}
\end{eqnarray*}
