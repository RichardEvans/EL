...
using single formulae, we can avoid structural inference rules.  \Cathoristic{}
...
entailment\martin{What does ``in terms of entailment'' mean?}) formula
that describes the model.  The function $\SIMPL{\cdot}$ takes formulae
...
$\CHAR{\MMM}$. The formula $\CHAR{\MMM}$ characterises fully the model
...
\NI Soundness is immediate from the definitions. 
 To prove complness  we will show that $\phi
