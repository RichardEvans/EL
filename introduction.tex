...
\begin{itemize}
\item
For every logical concept, we need a logic in which it can be expressed naturally (ergonomically)
\item
...
It is the only logic we have found that has a linear-time decision procedure and that satisfies:
\begin{itemize}
\item
The Hennessy-Milner theorem
\item
Brandom's Incompatibility Semantics property
\end{itemize}
\end{itemize}
