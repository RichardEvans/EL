...
easily expressed in first-order logic\footnote{We will precisify this
  ...
  exclusion is longer in terms of formula length than \ELFULL{}'s (see
  Section \ref{incompatiblepredicatesinfol}); and (ii) logic programs
  in \ELFULL{} can be optimised to run significantly faster than their
...
\NI But before they make\martin{``before making''?} this optional
...
conceptually \emph{prior}.  The rest of the paper elaborates on
Brandom's understanding of material incompatibility, and identifies as
well as formalises exclusion as its conceptual core.
...
language would look like.  EL is one way of formalising inferences
between atomic sentences.

\subsection{Outline}

\NI The rest of this paper is organised as follows: Section
\ref{coreEL} introduces the syntax and semantics of eremic logic with
examples. Section \ref{naturalLanguageInference} discusses how how
eremic logic can be used to model adverbial inferences and inferences
from dyadic to monadic predicates.  Section \ref{kr} describes
informally how eremic logic is useful as a knowledge representation
...
equivalence and a decision procedure with linear time-complexity. The
...
holds for eremic logic.  Section \ref{elAndBangMore} presents the
proof rules for eremic logic and proves completeness. Section
\ref{compactness} provides two translations from eremic into
first-order logic, and proves compactness using one of them.  Section
\ref{ELAndNegation} investigates the role of negation in eremic logic
in two ways: by introducing a variant of eremic logic with negation,
...
relates eremic logic and Hennessy-Milner Logic.  Section
\ref{quantifiedEL} extends eremic logic with first-order quantifiers
and sketches the translation of first-order formulae into first-order
eremic logic. The conclusion surveys related work and lists open
problems.  Appendix \ref{pureModels} outlines a different approach to
giving the semantics of eremic logics, including a characterisation of
