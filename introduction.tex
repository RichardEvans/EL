...
This paper introduces an alternative, \ELFULL{}, where exclusion is
...
\]
\martin{this is actually structurally the same example as pierre before.
Let's drop one}
This is simpler (both in terms of representation length and
computational complexity) than the representation\martin{representation twice in one sentence ... not good} in predicate logic:
\[
...
easily expressed in first-order logic\footnote{We will precisify this
  claim in later sections by showing\martin{we don't show that mathematically, ,amybe we need to tone down our language} that (i) first order logic's
...
\martin{Maybe we should weaken the title to ``Brandom's argument that
  exclusion is ...'' because later we say ``In the rest of this paper,
  we assume - without further argument - that material incompatibility
  is conceptually prior ...''}
...
\end{quote}
According to the alternative, pragmatist strategy that Sellars and Brandom propose, \martin{we use propose to often. Maybe use ``In Sellars and Brandom's alternative, pragmatist strategy ...''}
``Jack
...
claim that we can\martin{``can'' can be dropped} understand incompatible predicates even if we do
...
primordial language of atomic sentences\footnote{A sentence is atomic if it does not contain another sentence as a syntactic constituent. See Section \ref{intrasentential} below for a discussion.}\martin{why do we
make a forward reference for this but not for a lot of other concepts?}\richard{What do you suggest instead?}\martin{I think we should 
use forward references (in the introduction) only very sparingly, if at all. I think we an expect the reader to understand that the introduction is just an
introduction and lateron we expand on the subjects sketched here.}. These people can express
...
sophisticated language where incompatibilities and entailments can be\martin{``is made explicit'' }
...
\end{quote}
But before they make\martin{``before making''?} this optional explicitating step, our primitive people understand incompatibility without understanding negation.
...
\martin{We have not made good, except in the most subtle of ways, on the title of the subsection's promise that exclusion is prior to ... We have only
talked about material incompatibility}

\subsection{Negation arises as the logically weakest form of exclusion}
...
\item
There may be no person\martin{nobody?} answering to the name ``Jack''
\end{itemize}
All the above\martin{``These possibilities ...''} possibilities are concrete propositions which are incompatible with Jack supporting Manchester United.
To say that \martin{``that'' can be dropped here?} ``Jack does not support Manchester United'' is just to claim that one of these indefinitely many concrete possibilities is true.
...
We develop a simple
 modal logic to articulate Brandom's intuition formally: a language, without negation, in which we can make incompatible claims\footnote{Although this logic is inspired by Brandom's thoughts, this is not to say that he himself would endorse the approach described in this paper. As a pragmatist, Brandom stresses that dispositions or practices are prior to any \martin{``any'' can be dropped?} logical codification - so he might not see the need for, or wisdom of, attempting to \martin{``attempting to'' and be dropped and replaced by ``formalising''?}formalise these\martin{``these'' can be dropped?} pre-logical practices in a formal logic.\martin{``in a formal logic'' can be dropped.}}.
...
In this subsection, we shall describe \emph{other} inferential relations between atomic sentences - inferential relations that predicate logic cannot articulate, but that \ELFULL{} is able to handle\martin{``EL handles easily/naturally/convincingly''?}.

The \emph{atomic sentences} of a natural language can be
characterised as\martin{drop ``can be
characterised as'', replace by ``are''?} the sentences which do not contain any \martin{drop ``any''?} other
...
to this criterion, the following are atomic:\martin{Maybe simpler ``In that/this sense the sentences jackl ... jack ... are atomic. In contrast... the following is not because ...'' or something like that?}
...
syntactic constituent.  Note that, according to this criterion\martin{I think ``according to this criterion'' can be dropped}, the
...
\martin{So far we pitched EL as formalising Brandom's vision. Now it's LW's. Maybe it's better to say that as an additional benefit, EL \emph{also}
elaborates LW's vision?}

\subsection{Outline}\martin{I suggest to consider the outline in flowing text, not bulleted. and with concrete chapter refrences , Like
so. Section 2 introduces the syntax and sem,antics of EL, Sections 3 and 4 da da da, In section 4 ...
We should also say something along the lines of: A Haskell implementation of the decision procedure can be found in 
