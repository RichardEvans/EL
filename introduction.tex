...
Brandom \cite{brandom2} proposes an alternative explanation: ``Jack
is male'' is incompatible with ``Jack is female'' because ``is male''
and ``is female'' are \emph{materially incompatible} predicates.  He
claims that we can understand incompatible predicates even if we do
not understand universal quantification or negation.  In summary:

\begin{quote}Material
incompatibility is conceptually prior to logical negation.
\end{quote}

\NI Imagine, to make this vivid, a primitive people speaking a
primordial language of atomic sentences. These people can express
sentences that \emph{are} incompatible.  But they cannot express
\emph{that} they are incompatible or entail each other.  They
recognise when atomic sentences are incompatible, and see that one
sentence entails another, and their behaviour outreaches their ability
...
made explicit, for example using the negation and entailment operators
respectively.

Now imagine a variation of our primitive language in which no
...
back down, retract their claims, or justify them. They just make
``assertions''\footnote{We place sneer-quotes around ``assertion''
  because we believe, following Brandom, that in the absence of any
  sort of procedure for the recognition and resolution of incompatible
  claims, the sounds do not count as assertions at all.}.  Without an
understanding of incompatibility, and the variety of behaviour that it
...
If this further claim is also accepted, then material incompatibility
- exclusion - is not just conceptually \emph{independent} of logical
negation, but is conceptually \emph{prior} to it.

Whether an accurate picture of language development or not, Brandom's
argument is sufficiently fascinating to suggest investigating the
foundations of exlusion, i.e.~a logic without negation, implication
and disjunction, but instead a primitive expressing
exclusion. \martin{This sentence needs to be improved.}

\textbf{rest of section, is unedited }

The speakers of the core primordial language understand
incompatibility relations between atomic sentences, but use no complex
logical connectives.


  This language contains atomic sentences, but has
