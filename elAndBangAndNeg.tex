...
\ACTIONS{\phi}$ and $\ACTIONS{!A} = A$.\martin{Is that what you have
  in mind for $\ACTIONS{.}$?},
...
\end{itemize}
\end{enumerate}
\martin{Is that a correct rephrasing of the algorithm?}
\martin{I am not fully convinced that this algorithm is exponential:
  the conversion to DNF has a worst-case exponential blow-up, and the
  negation removal also increases formula size.  I *think* it's ok, it
  is exponential, but it's not totally trivial to see that it is.}
...
\end{itemize}
\end{definition}
\martin{Can that definition be right? No relations between $\lambda$
  and $\lambda'$?}
...
restricting the height of each tree to $|\psi|$.\martin{While that is correct, we don't have
