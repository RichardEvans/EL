...
\NI For example, let $\phi$ be $\langle a \rangle \top$ and $S = \Sigma = \{a,
b\}$.  Then

\[
   \neg_\Sigma(\langle a \rangle \top) 
      \quad= \quad 
   ! \{b\} \lor \langle a \rangle \neg_\Sigma(\top) 
      \quad =\quad  
   ! \{b\} \lor \langle a \rangle \bot
\]

\NI The term on the right is equivalent to $! \{b\}$. So, in this case:
\[
   \phi \lor \neg_{\Sigma} \phi 
       \quad=\quad 
...
But if we restrict our attention to the right kind of models then  excluded middle holds:
\[
   !\{a\} 
      \models \; 
   \langle a \rangle \top \; \lor \; ! \{b\} \; \lor \; \langle a \rangle \bot
...
\subsection{A Naive Decision Procedure}
\martin{Do we need this section?}
...
\subsection{A More Efficient DecisionProcedure}
