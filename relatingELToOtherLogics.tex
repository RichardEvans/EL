...
\item Semantically, an \ELABR{} model involves one restriction from and one addition to a standard HML labelled transition system. The restriction is that all \ELABR{} model are \emph{deterministic}, while many HML models are nondeterministic. The addition is the additional state-labelling on \ELABR{} models, a labelling that restricts the allowable transitions emanating from each vertex.
...
However, note that the following natural extension is \emph{not} true under the translation above:
\[
...
\]
Now this entailment is valid in EL because of the restriction to deterministic models, but is not valid in HML, 
where it is invalidated by any model with two outgoing $a$ transitions, one of which satisfies $\MAY{b}$ and one of which satisfies $\MAY{c}$.

However, if we add a determinism constraint to our translation, we can restore the desired connection between \ELABR{} implication and HML implication. Given a set $\Gamma$ of \ELABR{} formulae, closed under sub formulae, that contains actions from the set $A \subseteq \Sigma$, let the determinism constraint for $\Gamma$ be:
\[
\bigwedge_{a \in A, \phi \in \Gamma, \psi \in \Gamma} \neg \; (\MAY{a} \phi \land \MAY{a} \psi \land \neg \MAY{a} (\phi \land \psi) )
\]
If we add this sentence as part of our translation $\SEMB{}$, we do get the desired result that
\[
\text{If } \phi \models \psi \text{ then } \SEMB{\phi} \models \SEMB{\psi}
\]


\subsection{Why HML Without Negation Does Not Satisfy the Hennessy-Milner theorem}
