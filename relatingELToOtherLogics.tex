\section{Relating \cathoristic\ to Hennessy-Milner logic}\label{relatingELToOtherLogics}
\martin{we don't do much on propositional logic in this chapter, so
  title is misleading.  What about merging this section with the next
  on quantified CL? Or maybe better with the previous sectino?}
\richard{For now, I have just removed the reference to propositional
  logic in the section heading. I am not sure where we should place
  this section. We could place it in the conclusion (in the part where
  we compare CL to various other logics?)}\martin{Yes, that might not be a bad place.}
