...
female'' is translated into \ELABR{} as the pair\martin{what do we mean by ``pair'' here? Conjunction?} of incompatible
sentences:
\begin{eqnarray*}
...
\subsection{Representing incompatible predicates in predicate logic}\label{incompatiblepredicatesinfol}

\martin{why ``predicate logic'' here. Why not stick to ``first-order logic''?}
...
sentences:\martin{I dont' think we need to repeat them all as
  bullets. Or at least we can put them in the flowing text. }
\begin{itemize}
...
between predicate logic and our ergonomic mother tongue,\martin{A tad polemical ...} to be exactly
...
%We plan to address this representational issue in future
%work. \martin{no need to make promises. Only the facts ...}
