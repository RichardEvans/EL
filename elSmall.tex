...
\psi$ is the conjunction of formulae $\phi$ and $\psi$. 
Asserting that $\MAY{a}{\phi}$ is stating that the current state
can transition via action $a$ to a new state at which
$\phi$ holds. Tantum\footnote{``Tantum'' is Latin for ``only''.} $A$, 
...
Hence, $\Sigma$ must be
non-empty. 
Note that in the absence of conventional negation, we cannot
