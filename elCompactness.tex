...
\subsection{The standard translation from  \cathoristic{} into 
...
corresponds to an cathoristic model. The problem is that cathoristic models have
...
the point of view of \cathoristic{}.  We deal with this problem following
...
an cathoristic transition system $\LLL$, produces a corresponding
...
to cathoristic transition systems.  For example the constraint $s \TRANS{a}
t$ implies $a \in \lambda(s)$ might be violated. In the case of \cathoristic{}, 
merely having signature $\SSS$ is not strong enough a
...
ensure that they always can be translated back to \cathoristic{}.  The
second embedding translates \cathoristic{} to two-sorted first-order logic
...
\NI We use the translation $\SEMBTWO{\phi}_x$ to show that \cathoristic{}
...
that is the translation of an cathoristic model.
...
models are not directly convertible to cathoristic models. We solve these
...
\]
Since $\Gamma'_{EL}$ is finite, by assumption there is an cathoristic model 
\[
