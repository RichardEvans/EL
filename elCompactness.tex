...
This section studies embeddings of eremic logic into first-order
logic, and proves that eremic logic has compactness using one.

\subsection{The standard translation from  \ELABR{} into 
            first-order logic}\label{standardTranslation}

Studying a new logic is interesting not just for the concepts the
operations of the logic formalise, but also because logics do not live
in isolation. We can study how a logics embeds into other logics.
This helps explain not just the new logic, but also casts a new light
onto the the logic that is the target of the embedding. For example
the standard translation of modal logic into first-order logic gives
rise to interesting fragments like the finite variable fragments, the
...
\NI We now translate EL into a restricted fragment of first-order
logic (cf.~Figure \ref{figure:embedding}). 
...
t$ implies that $a \in \lambda(s)$. Such models contain 'junk' from
the point of view of \ELABR{}.  We deal with this problem following
ideas from modal logic \cite{BlackburnP:modlog}: we add a translation
$\SEMB{\LLL}$ for eremic transition systems, and then prove the
following theorem.
...
of models are identical,f save for details of prensentation.
...
to eremic transition systems.  For example the constraint $s \TRANS{a}
t$ implies $a \in \lambda(s)$ might be violated. In the case of eremic
logic, merely having signature $\SSS$ is not strong enough a
constraint. The target language of the translation from the previous
section is not expressive enough to have formulae that can guarantee
such constraints.  As we have no reason to belive that the first-order
...
can constrain first-order models easily using first-order formulae to
ensure that they always can be translated back to \ELFULL{}.  The
second embedding translates \ELFULL{} to two-sorted first-order logic
(which can easily be reduced to first-order logic
\cite{EndertonHB:matinttl}).
...
\begin{definition}
The encoding $\SEMBTWO{\phi}_x$ of \ELABR{} formulae is given by the following clauses.
\begin{eqnarray*}
  \SEMBTWO{\top}_x & \ = \ & \top
     \\
...
\NI We use the translation $\SEMBTWO{\phi}_x$ to show that \ELABR{}
must also have compactness. The key steps in the proof are simple,
following standard techniques from modal logic
...
\item By compactness of (two-sorted) first-order logic, we can find a
  first-order model $\CAL{M}$ of $\SEMBTWO{T}$.
...
be admissible. Likewise we use a formula $\phi_{det}$ to ensure model
determinism.
\begin{eqnarray*}
   \phi_{det} 
      & \ =\ &
...
because nothing we have discussed so far prevents models whose node
labels are infinite sets that are not $\Sigma$.  Moreover, a model may
interpret the set of actions with a proper superset of $\Sigma$.  Such
models are not directly convertible to eremic models. We solve these
problems by simply removing all actions that are not in $\Sigma$ and
all transitions involving such actions.  Moreover, we simply map all
infinite node-labels to $\Sigma$. It is easy to see that this does not
change satisfiability of (translations of) eremic formulae.
...
\begin{proof}
The proof proceeds by induction on the structure of $\phi$. \martin{To do. But it's obvious}
\end{proof}

\begin{definition}
Let $\Gamma$ be a set of \ELABR{} formulae, and $\MMM$ an eremic model.  We
...
A set $\Gamma$ of \ELABR{} formulae is satisfiable iff each finite subset of
$\Gamma$ is satisfiable.
\end{theorem}
\begin{proof}
For the non-trivial direction, let $\Gamma$ be a set of \ELABR{} formulae
...
$\Gamma^*$ has a first-order model. Why? First consider the subset $\Gamma'_{EL}$ of $\Gamma'$
which is given as follows.
\[
   \Gamma'_{EL} \ =\ \{ \phi \in \Gamma\ |\ \SEMBTWO{\phi} \in \Gamma' \}
\]
Since $\Gamma'_{EL}$ is finite, by assumption there is an eremic model 
\[
   (\LLL, s) \models \Gamma'_{EL}
\]
which means we can apply Theorem \ref{correspondence:theorem:223} to get
\[
   \SEMBTWO{\LLL} \models_{x \mapsto s} \SEMBTWO{\Gamma'_{EL}},
\]
By construction $\Gamma' \setminus \SEMBTWO{\Gamma'_{EL}} \subseteq
