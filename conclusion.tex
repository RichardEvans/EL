...
``complex'' logical connectives.  By ``complex'', we mean logical
...
be satisfied in different ways.  This condition was made precise in
Section \ref{boundedlattice}:\martin{We say nothing about simplicity
  in that section!} a logic is ``simple'' if the set of satisfying
models of every sentence has a unique least upper bound.  Disjunction
...
As is well known, the existential quantifier of Predicate Logic can be
translated into an infinitary disjunction of propositions in
Propositional Logic.  Analogously, given a set $\Sigma$ of symbols, a
negated formula in proposition logic can be translated into a
disjunction of materially incompatible formulae in \cathoristic{}.
\Cathoristic{} is an attempt to formalise Sellars' and Brandom's view
that material incompatibility is conceptually prior to logical
negation.

\martin{I don't think the two paragraphs above should be in the
  conclusion because they essentially develop new material. The
  conclusion is not the right place for that, except as a suggestion
  for further work. }
...
\Cathoristic{} started as an attempt to formalise the Sellars/Brandom
idea that material incompatibility is conceptually prior to logical
negation.\martin{Actually ... CL started as an attempt to formalise the Sellars/Brandom
idea OF material incompatibility ... we don't make arguments for priority. If anything, 
our translation into FOL could be seen as (weak) arguments against priority}  Its formal presentation and development owes much to modal
...
\subsubsection{Peregrin}\label{peregrin}
