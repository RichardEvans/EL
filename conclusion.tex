...
In \cite{peregrin}, Peregrin provided a set of general structural
rules that any logic must satisfy if it is to connect incompatibility
($\mathsf{Inc}$) and entailment ($\models$) via the Brandomian
constraint:
\martin{I would use present tense and write something like this:
...
\NI Peregrin showed that if a logic satisfied the above laws, then
incompatibility and entailment are mutually interdefinable, and the
logic satisfies the Brandomian constraint:
\[
X \models \phi \text{ iff } \mathcal{I}(\{\phi\}) \subseteq \mathcal{I}(X)
\]
\martin{We just cited this constraint above, no need for repetition}

\NI Next, Peregrin gave a pair of laws for defining negation in terms
...
\emph{minimal incompatible}\footnote{$\psi$ is the minimal
  ...
  \cup \{\xi\})$ then $\xi \models \psi$.}.  Now, in \cite{brandom},
Brandom defined negation slightly differently. He used the rule:
\begin{eqnarray*}
  (\neg B) & &\mathsf{Inc}(X, \neg \phi) \text{ iff } X \models \phi
\end{eqnarray*}
Using this stronger rule, we can infer the classical law of
double-negation: $\neg \neg \phi \models \phi$.  Peregrin established
...
\martin{As with the previous section, I think this section is too long and connectes only looselyw
...
more expressive.  Firstly, CL[$\land, !$], unlike either PL[$\land$]
...
$\langle a \rangle \top$ and $! []$).  Because CL[$\land, !$] is
...
\NI CL[$\land, !$] is the only logic we are aware of with a
...
think that Hennessy-Milner logic is more expressive than PL, and CL
