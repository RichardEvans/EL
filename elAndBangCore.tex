...
\NI It is easy to show, by induction on $\phi$, that
\[
...
decision procedure.  The complexity of the decision procedure shows
that \ELABR{} can be useful as a query language in knowledge
representation.

Since \ELABR{} has no connectives for negation, disjunction or
implication, its decision procedure is straightforward and efficient.
Although there are an infinite number of models which satisfy any
expression, we have shown that the satisfying models form a bounded
...
theorem:\martin{Am I right in assuming that the checker first computes $\SIMPL{\phi}$  and then
checks if $\SIMPL{\phi} \models \psi$? If so, may be we should say this explicityl.}
...
\cite{HasellImplementation}.  To see that the decision procedure is
linear-time, observe that every function in the Haskell implementation
is non-recursive and all iterative constructions use one
of \begin{verbatim}map, filter, fold, any, all \end{verbatim}- each of
which is linear in the size of its input.
