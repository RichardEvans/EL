...
is the semantic counterpart of the $!$ operator.  We restrict the
codomain of $\lambda$ so that $\lambda(w)$ is always either a finite
subset of $\Sigma$, or $\Sigma$ itself.\martin{we need to motivate
  this restriction!}
...
\begin{eqnarray*}
  \SEMB{\top}_x & \ = \ & \top  \\
  \SEMB{\phi_1 \AND \phi_2}_x & \ = \ & \SEMB{\phi_1}_x \AND \SEMB{\phi_2}_x  \\
...
eremic transition systems, and then prove the following correspondence
theorem.
...
an eremic transition system, produces a corresponding first-order
model for the signature $\SSS$.
...
\NI We are now ready to reap the benefits of reducing eremic to
first-order logic.
...
model exactly when every finite subset of $S$ does \cite[Chapter
  4.3]{EndertonHB:matinttl}. What about eremic logic?  We now use the
translation $\SEMB{\phi}_x$ to show that eremic logic must also have
compactness.  In order to carry out the proof, we need an inverse to
$\SEMB{\LLL}$.  Clearly we can also go the other way and start out
with a first-order model for the signature defined above, and convert
it to an eremic model.

\begin{definition}
Let $\CAL{M}$ be a first-order model for the signature $\SSS$ defined
in the previous section. We construct an eremic transition system
