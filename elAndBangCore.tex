...
\Cathoristic{} has the unusual property that every formula has a
unique (up to isomorphism) simplest model satisfying a formula.\martin{We need to say that
``simplest'' here means least w.r.t. $\MODELLEQ$. But if that's the case, isn't
it better to call it ``least'' model, to avoid multiplying equivalent terminology?}  Any
...
Before moving on to incompatibility semantics, we would like to point
to the Haskell implementation of the decision procedure
\cite{HaskellImplementation} and to sketch why \cathoristic{} has 
an efficient decision procedure.  

The reason is that it contains no ``complex'' logical connectives.
``Complex'' in this context means logical connectives that, generate
sentences that can be satisfied in different and substantially
distinct ways.  Negation, disjunction, implication, existential
quantification all give rise to formulae that are complex in this
sense.  As an example, disjunction is complex because there are two
equally \martin{equally simple, can we say that? What if $\phi = \top$
  and $\psi$ is a complex formula?} simple models of $\phi \lor
\psi$.\martin{We aslo need to distinbgusih this notion of simplicty
  from others we use elsewhere, e.g. $\SIMPL{.}$ and $\MODELLEQ$}
Negation is complex because there are two equally simple models of
$\neg (\phi \land \psi)$.  Conjunction is the \emph{only} connective
of propositional logic that is not complex, according to this
definition.  Let us define a logic as ``simple'' if the set of
satisfying models of every sentence has a unique least upper bound.
We saw in Theorem \ref{theorem:decision} of Section
\ref{boundedlattice} that \cathoristic{} is simple, according to this
definition.  It is because of this simplicity that it has a
quadratic-time decision procedure.  \martin{There are two separate,
  unconnected arguments: one about connective complexity, and one
  about logic simplicity. We should tie them together if poss,
  otherwise say there are two differnt reasons for the quadriatic
  compleixty.}
