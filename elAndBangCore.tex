...
element is $\bot$.  $\sqcap$ and $\sqcup$ are defined and shown to
have the required properties below. \martin{Actually ... the
  definitions of $\sqcap$ and $\sqcup$ below are explicit
  constructions. Semantically $\sqcap$ and $\sqcup$ are given in
  general from $\MODELLEQ$ and dont need an explicit definition, see
  Preliminaries.  What we should prove is that the LUBs and GLBs exist
  as required. And then later we can show that the algorithmic
  constructions below are correct w.r.t. to the semantic definitions.}
\end{proof}

\Cathoristic{} is unusual in that every formula has a unique (up to
isomorphism) simplest\footnote{The ``simplest'' model is the least
  upper bound according to the $\MODELLEQ$ relation.} model satisfying
a formula. Logics with disjunction, negation or implication do
not have this property.  For example, in propositional logic:
\begin{itemize}
\item
There are two equally-simple models satisfying $\phi \lor \psi$
\item
There are two equally-simple  models satisfying $\neg (\neg \phi \land \neg \psi)$
\item
There are two equally-simple models satisfying $\phi \Rightarrow \psi$
\end{itemize}
\martin{I suggest to change this. We have not defined the notion of
  model simplicity in propostional logic. Also for e.g. $\phi \OR
  \psi$ we have 3 different ways to make this true. Finally, ``There
  are two equally-simple models satisfying'' sounds repetitive and can
  be factored out.}
The existence of unique simplest models is crucial for proving
completeness of our inference rules, and in the quadratic-time
decision procedure.

\subsection{The simplest model satisfying a formula}
\label{simpl}

\martin{What we are in essence doing is give two names for the
  simplest model satisfying $\phi$: (1) simplest model, and (2) lub of
  all models. Do we need two names? Why not just stick with LUB? I
  also suggest to use an explicit definition for defining what we
  mean. And then we can say ``Definition ... above defined the
  simplest model/lub semantically.  Now we given an explicit
  construction. That would also make for a better subsection title,
  e.g.  ``Constructing the simplest ...''  BTW, Modal logic has a name
  for a related concept: the ``small model property''. Maybe we can
  tie in our presentation with this?}

Given $\MODELLEQ $, we can define $\SIMPL{\phi}$, the simplest model
w.r.t.~$\MODELLEQ $ that satisfies $\phi$.
...
\subsubsection{$\sqcap$ is the greatest lower bound}
We will show that:
\begin{itemize}
\item
...
\subsection{Least upper bound ($\sqcup$) of models}
\martin{WHy do we have a subsection fo lub but not for glb?}
...
\begin{eqnarray*}
  \mathsf{lub}(\CAL{L}, \CAL{L}', \MMM, \emptyset) 
     &\quad =\quad & 
  \MMM \\
...
\NI Here $\lambda[x \mapsto S]$ is the node labelling function that is
...
\subsubsection{$\sqcup$ is the least upper bound}
We will show that:
\begin{itemize}
\item
...
do.\martin{Can we still say this in the light of Peregrin's paper?}
