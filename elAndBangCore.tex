...
\begin{itemize}
\item
Lemma \ref{first-completeness-lemma}: if $m \models p$ then $\theta(m) \judge p$.
\item
Lemma \ref{second-completeness-lemma}: for all formulae $p$, $p \judge \theta(\mu(p))$.
\end{itemize}
...
\qed
\end{proof}
Next we will prove Lemma \ref{first-completeness-lemma}.
\begin{lemma}
\label{first-completeness-lemma}
...
\begin{lemma}
\label{second-completeness-lemma}
...
\]
Lemma \ref{third-completeness-lemma}, proven below, states that, for all models $m$ and $n$:
\[
\theta(m) \AND \theta(n) \judge \theta (m \sqcap n)
\]
From Lemma \ref{third-completeness-lemma} (substituting $\mu(q)$ for $m$ and $\mu(q')$ for $n$), it follows that:
\[
...
\begin{lemma}
\label{third-completeness-lemma}
...
This completes Lemma \ref{third-completeness-lemma} and hence the Completeness Proof.
\qed
