...
The complexity of the decision procedure shows that \ELABR{} can be useful as a
query language in knowledge representation. 

\NI Since \ELABR{} has no connectives for negation, disjunction or implication, its
...
satisfying models form a bounded lattice with a least upper bound.  The $\SIMPL$
...
There is a Haskell implementation of the decision procedure at
\url{https://github.com/RichardEvans/EL}.
To see that the decision procedure is linear-time, observe that every function in the Haskell implementation is non-recursive and all iterative constructions use one of \begin{verbatim}map, filter, fold, any, all \end{verbatim}- each of which is linear in the size of its input.
