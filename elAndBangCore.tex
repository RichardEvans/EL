...
\subsection{Turning $\MODELLEQ$ into a bounded lattice}
...
\begin{eqnarray*}
  ...
  \begin{cases}
    ...
    \mathsf{merge}(\mathcal{L}, \mathcal{L}'', ids')  \\
    \text{ where }  \mathcal{L}'' = \mathsf{applyIds}(ids, \mathcal{L}') \text{ and } ids' = ids \cup \mathsf{getIds}(\mathcal{L}, \mathcal{L}', ids)
  \end{cases}
\end{eqnarray*}
...
\]
The $\mathsf{join}$ function takes the union of the two LTSs. Because we insist that the set of states in the two LTSs are disjoint, there is no possibility of clashes.
\[
\mathsf{join}((S, \rightarrow,\lambda), (S', \rightarrow', \lambda')) = (S \cup S', \rightarrow \cup \rightarrow', \lambda \cup \lambda')
\]
