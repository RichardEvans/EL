...
of first-order logic\martin{is that true?}. We use the notion of
simulation to define an order $\MODELLEQ$ on models, which gives rise
to a bounded lattice of models.

\begin{definition}
The binary relation $\MODELLEQ$ on models is defined by the following
clause:
\[
   \MMM \MODELLEQ \MMM'
      \qquad\text{iff}\qquad
   \{\phi\ |\ \MMM \models \phi \} \subseteq  \{\phi\ |\ \MMM' \models \phi \}.
\]
\end{definition}

\NI The definition of $\MODELLEQ$ makes reference to formulae. We are
now developing a purely model-theoretic characterisation, based on an
extension of the simulation relation of eremic transition systems.
...
\item Whenever $(s, t) \in \RRR$ then also $\lambda_2(t) \subseteq
  \lambda_1(s)$.
...
\item $\RRR$ is a simulation from $\LLL_1$ to $\LLL_2$ as eremitc transition systems.

\item  $(s_1, s_2) \in \RRR$. 

\end{itemize}

\NI We write $\MMM_1 \SIM \MMM_2$ provided there is a simulation from
$\MMM_1$ to $\MMM_2$.  
...
%% transitions of $\MMM'$ while respecting \martin{ strengthening?}  the
%% transitions-restrictions. \martin{explain why we go contravariant on
%%   the $\lambda$s in the definition on $\SIM$.}

\begin{theorem}
Let $\MMM_i = (\LLL_i, w_i)$ be two models with underlying eremic
transition systems $\LLL_i = (S_i, \rightarrow_i, \lambda_i)$. Then equivalent:
\begin{enumerate}

\item $\MMM_1 \MODELLEQ \MMM_2$.

\item $\MMM_1 \LEQ \MMM_2$.
...
in $\Sigma$, and $A$ over \martin{finite?} subsets of
