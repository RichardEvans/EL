...
paper.  Order-theory is explored in \cite{DaveyBA:intlatao}. For a
fuller account on labelled transition systems, see
e.g.~\cite{SassoneV:modcontac,HennessyM:Algtheop}. Bisimulations are
discussed in \cite{SangiorgiD:intbisac}. \cite{EndertonHB:matinttl}
is one of many books on first-order logic.
...
\emph{finite}, we use $A$ instead of $\Sigma$.  We say $\LLL$ is
\emph{deterministic} if $x \TRANS{a} y$ and $x \TRANS{a} z$ imply that
$y = z$. Otherwise $\LLL$ is \emph{non-deterministic}.  A labelled
transition system is \emph{finitely branching} if for each state $s$,
